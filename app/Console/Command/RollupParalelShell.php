...
SQL;
	...
	function reverse_created($task) {
		$this->output("CREATED: " . $task->jobHandle());
	}

	function reverse_status($task) {
		$this->output("STATUS: " . $task->jobHandle() . " - " . $task->taskNumerator() .
				"/" . $task->taskDenominator());
	}

	function reverse_complete($task) {
		$this->output("COMPLETE: " . $task->jobHandle() . ", " . $task->data());
	}

	function reverse_fail($task) {
		$this->output("FAILED: " . $task->jobHandle());
	}

	function reverse_data($task) {
		$this->output("DATA: " . $task->data());
	}
