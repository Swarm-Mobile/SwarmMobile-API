...
SQL;
    ...
    public function register($params = array()) {
        ...
        if ($params['password'] != $params['password_confirm'])
            throw new APIException(501, 'invalid_params', "Passwords don't match");
        ...
        if (!empty($params['screen_name']))
            $data['screen_name'] = $params['screen_name'];
        ...
        if ($memExists)
            throw new APIException(501, 'duplicate_entry', 'Email not available');

        $memExists = $this->ExpMember->findByUsername($data['username']);
        if ($memExists)
            throw new APIException(501, 'duplicate_entry', 'Username not available');
