...
class APIComponent
{
    ...
    {
        throw new Exception("The requested reference method don't exists", 404);
    }
                    ...
                    switch ($validator) {
                        case 'required':
                            if (empty($params[$param]))
                                throw new Exception("Param $param is required and was not found in the request.");
                            break;
                        case 'int':
                            if ((!is_numeric($params[$param])) || $params[$param] != (int) $params[$param])
                                throw new Exception("Param $param needs to be and int.");
                            break;
                        case 'numeric':
                        ...
                        case 'date':
                            try {
                                ...
                                throw new Exception("Param $param needs to be of the form yyyy-mm-dd");
                            }
                            ...
                            if ($date < $swarm_born && $date < $max_date)
                                throw new Exception("Param $param needs to be a date greater than 2013-01-01 and lower than today.");
                            if (!preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $params[$param]))
                                throw new Exception("Param $param needs to have the yyyy-mm-dd format");
                            break;
                        case 'date_interval':
                            ...
                            try {
                                ...
                                throw new Exception("Param end_date needs to be of the form yyyy-mm-dd");
                            }
                            if ($start_date > $end_date)
                                throw new Exception("Param end_date needs to be greater than start_date");
                            break;
                        case 'year':
                            if (!is_numeric($params[$param]) || $params[$param] < 2013 || $params[$param] > date('Y') + 1)
                                throw new Exception("Param $param needs to be a valid year after 2012");
                            break;
                        case 'month':
                            if (!is_numeric($params[$param]) || $params[$param] < 1 || $params[$param] > 12)
                                throw new Exception("Param $param needs to be a valid month");
                            break;
                        case 'datetime':
                            //TODO:
                            break;
...
SQL;
    ...
    public function format ($aRes, $data, $params, $endpoint, $t1, $t2, $dbAlias = 'value')
    {
        ...
        foreach ($aRes as $oRow) {
            ...
            if (
                    (int) $hour >= (int) strstr($data['data'][$weekday . '_open'], ':', true) &&
                    (int) $hour <= (int) strstr($data['data'][$weekday . '_close'], ':', true) && $isOpen
            ) {
                $cResult['data']['breakdown'][$date]['hours'][$hour]['open'] = true;
                @$cResult['data']['breakdown'][$date]['totals']['open'] += $cValue;
                @$cResult['data']['breakdown'][$date]['totals']['close'] += 0;
        ...
        foreach ($errors as $key => $values) {
            foreach ($values as $val) {
                ...
                elseif ($val === 'minLength' && $key == 'username')
                    throw new Exception("Username has to be atleast 3 characters long.");
                elseif ($val === 'minLength' && $key == 'password')
                    throw new Exception('Password needs to be atleast 5 characters long.');
            }
        }
    }
