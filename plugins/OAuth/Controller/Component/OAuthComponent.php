 ...
 */
	public function getRefreshToken($refresh_token) {
		$refreshToken = $this->RefreshToken->find('first', array(
			'conditions' => array('RefreshToken.refresh_token' => $refresh_token),
			'recursive' => -1
		));
		if ($refreshToken) {
 ...
 */
	public function checkUserCredentials($client_id, $username, $password) {
		$user = $this->User->find('first', array(
			'conditions' => array(
				'User.'.$this->authenticate['fields']['username'] => $username,
				'User.'.$this->authenticate['fields']['password'] => AuthComponent::password($password)
			),
			'recursive' => -1
		));
		if ($user) {
